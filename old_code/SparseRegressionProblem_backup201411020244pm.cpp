...
//        start to add cut !!!
    ...
    try {
        ...
        if ( ( (nnode < 1000) && ( criterior2 < -1e-2 ) )  || ( (nnode > 1000) && ( criterior2 < -1e-1 ) ) ) {
            ...
            cut = ( l - u - rhs >= 0 );
#if test_adap_psd_cut || test_adap_psd_cut2 || display_apsd_cut
            if (n <= 5)
            {
                cout << " valz =  " ;
                for (int i = 0; i < n; i++)
...
ILOLAZYCONSTRAINTCALLBACK7(myLazy, IloBoolVarArray,z, IloNumVar,f, IloNumVar,g, IloNumVar,mse, DataModel &,data, TuneParameters &, tune, vector<double> &, lastValz )
{
    ...
    try {
        ...
        cout << " ILOLAZYCONSTRAINTCALLBACK6 tag (e45hy56jkr67kiu4e) " << endl;
        cout << " valz = " << valz << endl;
#endif
...
void SparseRegressionProblem :: build_SR_Objective (IloModel &model, IloBoolVarArray &z, IloNumVar &f, IloNumVar &g , IloNumVar &mse )
{
    ...
    if (tune.kSparse == 0) {
        ...
        exprObj = m * mse + log(m)/2.0 * tune.sigma2Noise * exprSumz;
    } else {
        int k = tune.kSparse;
        IloExpr  rhs(env);
        for ( int j = 0; j < n; j++) {
