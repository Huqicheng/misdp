...
// 2014 12 01
#define test_adap_nsd_cut_l3 0
#define test_adap_nsd_cut_level2 0
#define test_adap_nsd_cut_level3_circ0 1

#define INFO_adaptive_psd 1
...
//                        ##    ##  ##    ##  ##     ## ##     ##          ##   ### ##    ## ##     ##
//                         ######   ##     ## ##     ## ########           ##    ##  ######  ########
//  use condition: A \circ B >= 0  <=>  A \circ B(0)>= 0, B<=0.
...
{
#if test_adap_nsd_cut_l3 || test_adap_nsd_cut_level3_circ0
...
#if test_adap_nsd_cut_level3_circ0
    ...
    // prepare for some constant matrix
    vector<vector<double>> JLJ( data.KUK(J,data.OriginalL) );
    vector<vector<double>> KUK( data.KUK(K,data.OriginalU) );

    vector<double> Dsqrt(n);
    for (int i = 0; i < n; i++)
        Dsqrt[i] = sqrt( 2*(JLJ[i][i] - KUK[i][i]) ) ;
    vector<double> Dinv_JLJcircJLJminusKUKcircKUK_D(n*n);
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            Dinv_JLJcircJLJminusKUKcircKUK_D[i*n+j] = JLJ[i][j]*JLJ[i][j] - KUK[i][j]*KUK[i][j] ;
    data.inversePSD(Dinv_JLJcircJLJminusKUKcircKUK_D, n);
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            Dinv_JLJcircJLJminusKUKcircKUK_D[i*n+j] *= Dsqrt[i] * Dsqrt[j] ;
    
    vector< vector< double > > LHS(n, vector<double>(n));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            LHS[i][j] = Dinv_JLJcircJLJminusKUKcircKUK_D[i*n+j] ;   // useful
    vector<vector<double>> XinvJinvLinvJX( data.KUK(Jinverse, data.KUK(sqrtOneMinusXhatOverXhat, data.OriginalLinverse)) );  // useful
    vector<vector<double>> XinvKinvUinvKX( data.KUK(Kinverse, data.KUK(sqrtOneMinusXhatOverXhat, data.OriginalUinverse)) );  // useful
#if test_adap_nsd_cut_level3_circ0
    cout << "\n\n\n";
    cout << " SparseRegressionProblem :: generateCuts_total_gradient_adaptive_nsd tag (fq34fq34gq34q3) " << endl;
    cout << "LHS = ["  << endl;
    for (int i = 0; i < n; i++){
        for (int j = 0; j < n; j++)
            cout << LHS[i][j] << ", " ;
    ...
    cout << "XinvJinvLinvJX = ["  << endl;
    for (int i = 0; i < n; i++){
        for (int j = 0; j < n; j++)
            cout << XinvJinvLinvJX[i][j] << ", " ;
        cout << "\n";
    }
    cout << " ] " << endl;
    
    cout << "XinvKinvUinvKX = ["  << endl;
    for (int i = 0; i < n; i++){
        for (int j = 0; j < n; j++)
            cout << XinvKinvUinvKX[i][j] << ", " ;
        cout << "\n";
    }
    cout << " ] " << endl;
    ...
    while (mu > 1e-6) {
        totalObjectiveOld = data.logDet_AplusX( s , XinvJinvLinvJX ) - data.logDet_AplusX( s , XinvKinvUinvKX ) + log(data.Sigma[n][n]);
        ...
        while (!flagFindOptimalinMu) {
            ...
            data.newtonDirectionPSDSeparationProblem(direction, s, XinvJinvLinvJX, XinvKinvUinvKX, LHS, mu) ;
            //            vector<double> direction ( n,1 );
            ...
            if (normDirection > n){
                for (int i = 0; i < n; i++)
                    direction[i] = direction[i] / normDirection * n ;
                normDirection = n;
#if test_adap_nsd_cut_level3_circ0
                cout << " we need to scale the direction !!!!  " << endl;
                cout << "direction = [ ";
                for (int i = 0; i < n; i++)
                    cout << direction[i] << ", " ;
                cout << " ] " << endl;
            ...
            if (normDirection < max(1e-1*mu, 1e-3)*n) {
                ...
                double SstepLength = 1;
                double SBestStepLength = 1;
                double totalObjectiveNew = totalObjectiveOld;
                bool flag = true;
                while ( flag && (SstepLength < 1000) ){
                    vector <double> news(n,0.0);
                    SstepLength *= 3;
                    for (int i = 0; i < n; i++){
                        news[i] = s[i] + SstepLength * direction[i];
                        if ( news[i] < 1e-5 ) {
                            flag = false;
                            break;
                        }
                    ...
                    if (flag) {
                        ...
                        totalObjectiveNew = data.logDet_AplusX( news , XinvJinvLinvJX ) - data.logDet_AplusX( news , XinvKinvUinvKX ) + log(data.Sigma[n][n]);
                        totalObjectiveNew += mu * data.logDet_AplusX( negatives, LHS );
                        for (int i = 0; i < n; i++)
                            totalObjectiveNew += mu * log(news[i]);

#if test_adap_psd_cut_level5 || test_adap_psd_cut2 || test_adap_nsd_cut_level3_circ0
                        ...
                        cout << " criterior = totalObjectiveNew - totalObjectiveOld - 0.01 * SstepLength = " << totalObjectiveNew - totalObjectiveOld - 0.01 * SstepLength << endl;
#endif
                        if ( totalObjectiveNew - totalObjectiveOld < 1e-2 * SstepLength)
                            flag = false;
                        else
                            SBestStepLength = SstepLength;
                    }
                };
                flag = true;
                SstepLength = SBestStepLength / .5;
                while (flag && ( SstepLength * normDirection > 1e-3 ) && ( SstepLength  > 1e-3 ) ){
#if test_adap_nsd_cut_level3_circ0
                    cout << " tag __FILE__ = " << __FILE__ << ", __LINE__ = " << __LINE__ << endl;
                    cout << " YstepLength = " << SstepLength << endl;
#endif
                    bool subFlag = true; // check if YstepLength is valid
                    SstepLength *= (1-.618);
                    vector < double > news(n);
                    for (int i = 0; i < n; i++){
                        news[i] = s[i] + SstepLength * direction[i];
                    }
#if test_adap_nsd_cut_level3_circ0
                    ...
                    if (subFlag) {
                        ...
                        cout << " YstepLength = " << SstepLength << endl;
                        cout << " totalObjectiveNew = " << totalObjectiveNew << ", totalObjectiveOld = " << totalObjectiveOld << endl;
                        cout << " criterior = totalObjectiveNew - totalObjectiveOld - 0.01 * YstepLength = " << totalObjectiveNew - totalObjectiveOld - 0.01 * SstepLength << endl;
#endif
                        ...
                        if ( totalObjectiveNew - totalObjectiveOld - 0.01 * SstepLength < 0){
                            // illegal
                            ;
                        } else {
                            SBestStepLength = SstepLength;
                ...
                if (flag) {
                    ...
                    for (int i = 0; i < n; i++)
                        s[i] = s[i] + SstepLength * direction[i];
                    totalObjectiveOld = totalObjectiveNew;
                    
#if test_adap_nsd_cut_level3_circ0
                    cout << " tag __FILE__ = " << __FILE__ << ", __LINE__ = " << __LINE__ << endl;
                    cout << " optimal YstepLength = " << SstepLength << endl;
...
#if test_adap_nsd_cut_level3_circ0 && 0
...
#if test_adap_nsd_cut_level3_circ0 || betaTest
...
//                        ##    ##  ##    ##  ##     ## ##     ##          ##   ### ##    ## ##     ##
//                         ######   ##     ## ##     ## ########           ##    ##  ######  ########
//  use condition: A \circ B >= 0  <=>  A<=0, B<=0.
void SparseRegressionProblem :: generateCuts_total_gradient_adaptive_nsd_old_negneg( IloEnv &env, IloConstraintArray &cuts2BeAdded, DataModel &data, TuneParameters &tune, IloBoolVarArray &z, IloNumVar &u, IloNumVar &l, IloNumArray valz, IloNum valu, IloNum vall, long nnode, vector<double>  &lastValz)
{
...
for (int loopKJ = 0; loopKJ < 20; loopKJ++) {
...
//                        for (int i = 0; i < n; i++) cout << J[i] << "\t"; cout << " ] " << endl;
    ...
    for(int i = 0; i<n; i++)// Jnew^2 = J^2S, Knew^2 = K^2S
        s[i] = 1;
...
//        start to add cut !!!
    ...
    try {
        ...
        //  test if current solution violates the cutting planes. if yes, add the cut
        double criterior2 = (vall - valu - c);
#if test_adap_psd_cut_level4 || test_adap_psd_cut2 || display_apsd_cut || INFO_adaptive_psd
